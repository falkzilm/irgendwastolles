...
describe('store persistence', () => {
  ...
  it('übernimmt ein explizit deaktiviertes hudEnabled', async () => {
    window.api = {
      loadPersistedState: vi.fn().mockResolvedValue({
        data: {
          theme: 'dark',
  ...
  it('ergänzt bei alten, vor IRGENDWAST-47 persistierten Daten ohne hudEnabled den Default true, statt die restlichen Werte zu verwerfen', async () => {
    window.api = {
      loadPersistedState: vi.fn().mockResolvedValue({
        data: {
          theme: 'dark',
